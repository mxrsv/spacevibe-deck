...
> **Status:** Approved and executed 2026-08-09, after review amended four
> defects (a referenced test file that did not exist, the CSS-precedence trap,
> the search bar's missing accessible names, and a red phase with no
> discriminator). Tasks 1–7 are implemented and verified; **Task 8, the
> production-fidelity visual approval, has NOT been done** — see the drift
> ledger in [`docs/ARCHITECTURE.md`](../ARCHITECTURE.md) `building`.
...
- `DeckIcon` owns `aria-hidden="true"`, `focusable="false"`, `fill="none"`, `stroke="currentColor"` and `strokeWidth={1.8}`. The containing button or link owns the accessible name. `1.8` is not a new value: the chrome and settings-rail icons already use `viewBox="0 0 24 24"` with `stroke-width="1.8"`, which is the same coordinate space Lucide draws in, so their weight carries over unchanged. The Open Board's two folder drawings are the exception — a 16-unit box at stroke 1.5/1.6 — and will come out lighter; that is a visual-review item, not a defect.
