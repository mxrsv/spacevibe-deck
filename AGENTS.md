...
- **The tag path builds Electron, on both platforms, since 2026-08-20.**
  [`electron-release.yml`](.github/workflows/electron-release.yml) `current` is four jobs —
  `prepare` validates the tag and runs the source gate once, `mac` and `windows` build and
  publish into one draft, `promote` requires all six updater assets and only then makes it
  public, `--latest` for a stable tag and `--prerelease` for an `-electron.N` one. The
  trigger tag is `build/vX.Y.Z…`; the RELEASE tag is that without the prefix and does not
  exist until the run finishes, because a pushed release tag advertises a version whose
  manifest is still ten minutes away. `release.yml` is `workflow_dispatch` only now, with
  every Tauri job and the `release-freeze` job kept so a hotfix stays possible by hand —
  though a manual run reaches the Windows preview alone until someone edits
  `build-macos-stable-draft`'s `push`-only condition. **Gate A is closed for macOS**
  (owner-verified self-update against `v0.12.5-electron.2`, 2026-08-19); **Gate C is not**,
  and Windows ships unsigned and unverified BY DECISION rather than by omission — the
  release notes say so, since a user cannot discover it from the app. Nothing here has run:
  no tag has been pushed through this workflow. See
  [design](docs/specs/2026-08-20-electron-stable-release-design.md) `decided` and
  [docs/CONTEXT.md](docs/CONTEXT.md#two-platforms-one-tag--the-stable-release-path--2026-08-20)
  `current`. Freeze prerequisites are still open: `package.json` is `0.12.5-electron.2` and
  must be set to the stable version being cut, and `npm test` must be green on the release
  commit — it is not green on this checkout.
...
- **The tag path ships Electron on two platforms, and `release.yml` loses its tag
  trigger (2026-08-20, owner-approved by "implement this spec").** Four fork-listed
  categories at once: bundle and release-channel configuration
  (`electron-builder.release.yml` gains `win:`/`nsis:`), signing (Windows ships
  UNSIGNED by decision — electron-builder warns and continues, so a signing step
  that did nothing would look identical to one that worked), updater configuration
  (`allowPrerelease` stays `true` past the stable release, for a NEW reason: the
  installed builds are `0.12.5-electron.2` and looking past a prerelease is exactly
  what the flag buys), and Electron/Tauri cutover scope. Chosen over a second
  workflow file for Windows: two files on one tag race to create the release and
  neither has a point where the full artifact set is known complete, which is what
  `promote` exists to be. `allowToChangeInstallationDirectory: true` is KEPT
  deliberately — it is how a user reaches the one Windows failure on record (an NSIS
  extract failure on a secondary drive), but removing it hides that bug rather than
  fixing it. A platform claim with no matching hardware: Windows install and update
  are **unverified** and shipped that way on the owner's decision, stated in the
  release notes rather than left silent.
...
| ----------------------------------------------------------------------------- | ---------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Electron can replace Tauri on both supported platforms                        | `building` | partial    | **Gate A is closed for macOS** — owner-verified self-update against `v0.12.5-electron.2`, 2026-08-19. Gate C still lacks a real Windows run, and the 2026-08-20 pipeline ships without one by decision — [detail](docs/CONTEXT.md#verification-state-ledger) `current`                                                                                                                                                                                                                                                                                                                                                                                      |
| Deck ships the Electron host                                                  | `decided`  | unverified | The tag path builds Electron as of 2026-08-20: `electron-release.yml` is four jobs covering macOS and Windows, and `release.yml` is `workflow_dispatch` only. **No tag has been pushed through it** — [detail](docs/CONTEXT.md#verification-state-ledger) `current`                                                                                                                                                                                                                                                                                                                                                                                         |
...
| One tag ships macOS and Windows                                               | `decided`  | unverified | Built 2026-08-20 to [this design](docs/specs/2026-08-20-electron-stable-release-design.md) `decided`: `electron-release-config.test.ts` 37/37 with five mutations each shown to fail it, plus `npm run build`, `electron:build` and `generate:menu:check` — but no run of this workflow exists — [detail](docs/CONTEXT.md#verification-state-ledger) `current`                                                                                                                                                                                                                                                                                              |
| The Windows build installs and self-updates                                   | `decided`  | unverified | No Windows hardware; the owner elected to ship without the check (Gate C). The `windows` job proves only that the installer and `latest.yml` were PRODUCED — [detail](docs/CONTEXT.md#verification-state-ledger) `current`                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| macOS self-updates                                                            | `current`  | verified   | Owner-run check against `v0.12.5-electron.2`, 2026-08-19: reported the newer version, downloaded it, relaunched on it. Gate A's evidence, and it says nothing about Windows — [detail](docs/CONTEXT.md#verification-state-ledger) `current`                                                                                                                                                                                                                                                                                                                                                                                                                 |
| The Windows installer is code-signed                                          | `current`  | **false**  | Unsigned by decision (2026-08-19); SmartScreen warns on first install until reputation accrues. Stated in the release notes, since a user cannot discover it from the app — [detail](docs/CONTEXT.md#verification-state-ledger) `current`                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Intel Macs and Windows ARM are served                                         | `current`  | **false**  | macOS arm64 and Windows x64 only. Intel Mac users of the Tauri build have no upgrade path at all, which the last Tauri release has to say — [detail](docs/CONTEXT.md#verification-state-ledger) `current`                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Settings and workspaces carry over from Tauri                                 | `current`  | **false**  | Clean install by decision: two bundle identities, two userData directories, no updater between them. The final Tauri release must name the old path — [detail](docs/CONTEXT.md#verification-state-ledger) `current`                                                                                                                                                                                                                                                                                                                                                                                                                                         |
