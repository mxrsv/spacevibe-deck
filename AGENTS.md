...
- **Tags ship Electron on both platforms now; Tauri is retired from tag triggers
  ...
  **Gate A is CLOSED for macOS** — owner-verified auto-update against `v0.12.5-electron.2`,
  ...
  shipped publicly, so the stable is the first public macOS release. The pipeline itself has
  never run; no stable tag is cut. See
  [spec](docs/specs/2026-08-20-electron-stable-release-design.md) `decided` and
...
| Deck ships the Electron host                                                  | `decided`  | backlog    | The tag path builds Electron for both platforms since 2026-08-20 (`release.yml` is dispatch-only), but no stable tag is cut and the pipeline has never run — [detail](docs/CONTEXT.md#verification-state-ledger) `current`                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| One pushed tag ships macOS and Windows, both self-updating                    | `building` | unverified | Pipeline built 2026-08-20, never run. Windows unsigned (SmartScreen warns) and runtime-unverified (Gate C); preview.2 on Windows updates into a side-by-side install; Intel Mac and Windows ARM not served — [detail](docs/CONTEXT.md#verification-state-ledger) `current`                                                                                                                                                                                                                                                                                                                                                                                  |
