...
exports.createUpdateLifecycle = createUpdateLifecycle;
function releaseNotesText(notes) {
    ...
    if (Array.isArray(notes)) {
        const joined = notes
            .map((entry) => typeof entry === "object" && entry !== null && "note" in entry
            ? String(entry.note ?? "")
            : "")
...
function createUpdateLifecycle(deps) {
    ...
    const download = () => {
        ...
        if (downloadInFlight !== null) {
            ...
            return downloadTarget === target
                ? downloadInFlight
                : Promise.reject(new Error(`Deck is already downloading ${downloadTarget ?? "another update"}.`));
        }
        downloadTarget = target;
        downloadInFlight = new Promise((resolve, reject) => {
        ...
        load()
            .downloadUpdate()
            .then(() => settleDownload())
            .catch((error) => downloadSettle?.reject(asError(error)));
    ...
    const install = () => {
        ...
        installInFlight = new Promise((_resolve, reject) => {
