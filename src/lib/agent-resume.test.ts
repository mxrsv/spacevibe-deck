...
describe("buildResumeCommand — claude", () => {
  it("id ref", () => {
    expect(
      buildResumeCommand("claude", { kind: "id", id: "abc123" }, NO_CUSTOM),
    ).toBe("claude --resume abc123");
  });
  it("latest ref", () => {
    expect(buildResumeCommand("claude", { kind: "latest" }, NO_CUSTOM)).toBe(
      "claude --continue",
    );
  });
  it("null ref", () => {
...
describe("buildResumeCommand — codex", () => {
  it("id ref", () => {
    expect(
      buildResumeCommand("codex", { kind: "id", id: "abc123" }, NO_CUSTOM),
    ).toBe("codex resume abc123");
  });
  it("latest ref", () => {
...
describe("buildResumeCommand — opencode", () => {
  it("id ref", () => {
    expect(
      buildResumeCommand("opencode", { kind: "id", id: "abc123" }, NO_CUSTOM),
    ).toBe("opencode -s abc123");
  });
  it("latest ref", () => {
    expect(buildResumeCommand("opencode", { kind: "latest" }, NO_CUSTOM)).toBe(
      "opencode -c",
    );
  });
  it("null ref", () => {
...
describe("buildResumeCommand — gemini", () => {
  ...
  it("id ref falls back to the latest form (id never produced for gemini)", () => {
    expect(
      buildResumeCommand("gemini", { kind: "id", id: "abc123" }, NO_CUSTOM),
    ).toBe("gemini --resume latest");
  });
});
...
describe("buildResumeCommand — agy", () => {
  it("id ref", () => {
    expect(
      buildResumeCommand("agy", { kind: "id", id: "abc123" }, NO_CUSTOM),
    ).toBe("agy --conversation abc123");
  });
  it("latest ref", () => {
    expect(buildResumeCommand("agy", { kind: "latest" }, NO_CUSTOM)).toBe(
      "agy --continue",
    );
  });
  it("null ref", () => {
...
describe("buildResumeCommand — degradation", () => {
  ...
  it("refuses an id over the length cap", () => {
    ...
    expect(
      buildResumeCommand("claude", { kind: "id", id: tooLong }, NO_CUSTOM),
    ).toBe("claude");
  });
  it("accepts an id at the length cap", () => {
    const atCap = "a".repeat(128);
    expect(
      buildResumeCommand("claude", { kind: "id", id: atCap }, NO_CUSTOM),
    ).toBe(`claude --resume ${atCap}`);
  });
