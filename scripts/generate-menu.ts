...
function tokenFor(binding: KeyBinding): string {
  return "code" in binding ? normalizeCode(binding.code) : normalizeKey(binding.key);
}
...
function buildConstTable(constName: string, submenu: MenuSubmenu): string {
  const rows = itemsFor(submenu)
    .map((item) => `    ("${item.id}", "${item.label}", ${acceleratorFor(item.id)}),`)
    .join("\n");
...
if (process.argv.includes("--check")) {
  ...
  const committedScratchPath = join(scratchDir, "committed_menu_registry.rs");
  try {
