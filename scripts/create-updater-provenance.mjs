...
export function buildUpdaterProvenance(fileContents, sourceSha, tag) {
  ...
  invariant(/^v\d+\.\d+\.\d+-windows-preview$/.test(tag), "Preview tag is invalid");
  ...
  const files = Object.fromEntries(
    names.map((name) => {
      ...
      return [
        name,
        createHash("sha256").update(fileContents[name]).digest("hex"),
      ];
    }),
  );
  ...
  const fileContents = Object.fromEntries(
    await Promise.all(
      entries.map(async (entry) => [
        entry.name,
        await readFile(resolve(directory, entry.name)),
      ]),
    ),
  );
  const provenance = buildUpdaterProvenance(
    fileContents,
    args.sha,
    args.tag,
  );
